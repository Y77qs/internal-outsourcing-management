...
public record WorkLogUpdateRequest(
        ...
        @Schema(description = "工作内容", example = "补充接口联调记录")
        @NotBlank(message = "工作内容不能为空")
        @Size(max = 2000, message = "工作内容不能超过 2000 个字符")
        String workContent,

        @Schema(description = "问题记录", example = "发现移动端表格需要滚动容器")
        @Size(max = 1000, message = "问题记录不能超过 1000 个字符")
        String issueRecord,

        @Schema(description = "完成情况", example = "已完成并提交复测")
